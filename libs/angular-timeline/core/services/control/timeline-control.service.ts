...
import { fromEvent, merge, Observable, OperatorFunction, pipe, UnaryFunction } from 'rxjs';
import { filter, map, tap, withLatestFrom                                    } from 'rxjs/operators';
import { ElementRef, Injectable                                              } from '@angular/core';

import { TimelineConfig, TimelineControl, TimelineCamera, ViewBounds } from '@bespunky/angular-timeline/abstraction';
import { useActivationSwitch                                         } from '@bespunky/angular-timeline/shared';
...
export class TimelineControlService extends TimelineControl
{
    ...
    constructor(private config: TimelineConfig, private camera: TimelineCamera, private element: ElementRef)
    {
        super();
    ...
    private moveCamera<T extends EventWithModifiers>(
        getAmount: (event: T) => number
    ): UnaryFunction<Observable<T>, Observable<AcceleratedEvent<T>>>
    {
        return pipe(
            this.accelerateWithKeyboard<T>(getAmount),
            tap(([, amount]) => this.camera.move(amount))
        );
    }

    private zoomCamera<T extends EventWithModifiers>(
        getPosition: (event: T, viewBounds: ViewBounds) => number,
        getAmount  : (event: T) => number
    ): UnaryFunction<Observable<T>, Observable<CameraZoomParams>>
    {
        return pipe(
            this.accelerateWithKeyboard<T>(getAmount),
            withLatestFrom(this.camera.viewBounds),
    ...
    private zoomOnWheelFeed(): Observable<CameraZoomParams>
    {
        return this.wheel.pipe(
            useActivationSwitch(this.config.zoomOnWheel),
            filter(e => e.deltaY !== 0),
            this.zoomCamera(
    ...
    {
        return this.wheel.pipe(
            useActivationSwitch(this.config.moveOnWheel),
            filter(e => e.deltaX !== 0),
            this.moveCamera(e => Math.sign(e.deltaX) * this.config.moveAmount.value)
        );
    }

    private zoomOnKeyboardFeed(): Observable<CameraZoomParams>
    {
        const keydown = this.keydown.pipe(useActivationSwitch(this.config.zoomOnKeyboard));
    ...
    private moveOnKeyboardFeed(): Observable<AcceleratedEvent<KeyboardEvent>>
    {
        const keydown = this.keydown.pipe(
            useActivationSwitch(this.config.moveOnKeyboard)
        );

        const moveRight = keydown.pipe(filter(e => e.key === Key.ArrowLeft ), tap((e: any) => e.moveAmount = -this.config.moveAmount.value));
        const moveLeft  = keydown.pipe(filter(e => e.key === Key.ArrowRight), tap((e: any) => e.moveAmount =  this.config.moveAmount.value));

        return merge(moveLeft, moveRight).pipe(
            this.moveCamera((e: any) => e.moveAmount * this.config.moveAmount.value)
        );
    }
}